...
function getAssetsUri() {
    ...
    // Check if we are running from flutter_assets (common in mobile WebViews)
    var key = 'flutter_assets';
    var index = location.href.indexOf(key);
    if (index !== -1) {
        return location.href.substring(0, index + key.length) + '/';
    }
